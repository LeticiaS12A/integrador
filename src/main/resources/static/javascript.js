...
relatorioForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (relatorioIdEdicao) {
            ...
            const response = await fetch(`https://relata-facil.azurewebsites.net/a/pi`, {
                method: 'POST',
                headers: {
...
editForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const response = await fetch(`https://relata-facil.azurewebsites.net/a/${relatorioIdEdicao}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function listarRelatorio() {
    try {
        const response = await fetch('https://relata-facil.azurewebsites.net/a');
        ...
        relatorios.forEach(relatorio => {
			...
			<div class="relatorio-container">
			    <strong class="nomeReList">${relatorio.nomeRelatorio}</strong>
			    <div class="imagem-e-botoes">
			        <img id="imglist" src="${relatorio.urlImagem}" alt="${relatorio.nomeRelatorio}">
			        <div class="botoes-container">
			            <a class="img-edit" onclick="editarRelatorio(${relatorio.id})">
			                <img class="img-edit" src="img/editar-alt.svg" alt="botao de editar">
			            </a>
			            <a href="#" onclick="deletarRelatorio(${relatorio.id})">
			                <img class="img-delete" src="img/lixo.svg" alt="botao de deletar">
			            </a>
			        </div>
			    </div>
			</div>
...
function editarRelatorio(id) {
    ...
    fetch(`https://relata-facil.azurewebsites.net/a/${id}`)
...
async function deletarRelatorio(id) {
    if (confirm('Tem certeza que deseja deletar este relatorio?')) {
        try {
            const response = await fetch(`https://relata-facil.azurewebsites.net/a/${id}`, {
                method: 'DELETE'
            });
