...
relatorioForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (relatorioIdEdicao) {
            ...
            const response = await fetch(`https://relatafacil.azurewebsites.net/a/pi`, {
                method: 'POST',
                headers: {
...
editForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const response = await fetch(`https://relatafacil.azurewebsites.net/a/${relatorioIdEdicao}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function listarRelatorio() {
    try {
        const response = await fetch('https://relatafacil.azurewebsites.net/a');
...
function editarRelatorio(id) {
    ...
    fetch(`https://relatafacil.azurewebsites.net/a/${id}`)
...
async function deletarRelatorio(id) {
    if (confirm('Tem certeza que deseja deletar este relatorio?')) {
        try {
            const response = await fetch(`https://relatafacil.azurewebsites.net/a/${id}`, {
                method: 'DELETE'
            });
