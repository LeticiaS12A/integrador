...
relatorioForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (relatorioIdEdicao) {
            ...
            const response = await fetch(`https://db-leticia-leticia-f521.l.aivencloud.com:24566/a/pi`, {
                method: 'POST',
                headers: {
...
editForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const response = await fetch(`https://db-leticia-leticia-f521.l.aivencloud.com:24566/a/${relatorioIdEdicao}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function listarRelatorio() {
    try {
        const response = await fetch('https://db-leticia-leticia-f521.l.aivencloud.com:24566/a');
        ...
        relatorios.forEach(relatorio => {
            ...
            li.innerHTML = `
                ...
                <a class="img-edit" onclick="editarRelatorio(${relatorio.id})"><img class="img-edit" src="img/editar-alt.svg" alt="botao de editar"></a>
                <a href="#" onclick="deletarRelatorio(${relatorio.id})"><img class="img-delete" src="img/lixo.svg" alt="botao de deletar"></a>
            `;
...
async function deletarRelatorio(id) {
    if (confirm('Tem certeza que deseja deletar este relatorio?')) {
        try {
            const response = await fetch(`https://db-leticia-leticia-f521.l.aivencloud.com:24566/a/${id}`, {
                method: 'DELETE'
            });
